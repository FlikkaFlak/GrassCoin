GrassCoin-qt: Qt4 GUI for GrassCoin
===============================
...
Alternatively, install Qt Creator and open the `GrassCoin-qt.pro` file.

An executable named `GrassCoin-qt` will be built.
...
A warning for people using the *static binary* version of GrassCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of GrassCoin is linked against libdb5.3.
...
installed causes GrassCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
